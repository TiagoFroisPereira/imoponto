      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-foreground mb-6 tracking-tight">
            Quanto <span className="text-accent underline decoration-accent/30 underline-offset-8">dinheiro extra</span> quer no seu bolso?
          </h2>
            ...
            <Card className="border-2 md:border-4 border-primary/10 bg-card/50 backdrop-blur-sm shadow-xl">
              <CardContent className="pt-6 md:pt-8">
                <div className="space-y-4">
                  <label className="text-lg md:text-xl font-black text-foreground block">
                    Valor esperado de venda:
                  </label>
                  <div className="relative">
                    <Euro className="absolute left-4 md:left-5 top-1/2 -translate-y-1/2 w-5 h-5 md:w-7 md:h-7 text-accent" />
                    <Input
                      type="text"
                      ...
                      className="pl-12 md:pl-16 text-2xl md:text-3xl h-16 md:h-20 font-black border-2 focus-visible:ring-accent/20 bg-background border-border/50"
                    />
                  </div>
          ...
          <div className="lg:col-span-7 h-full">
            {parsedValue > 0 && totalAgencyCost > 0 ? (
              <Card className="relative h-full border-2 md:border-4 border-accent shadow-[0_0_50px_-12px_rgba(249,115,22,0.4)] bg-gradient-to-br from-accent/5 to-primary/5 overflow-hidden group">
                ...
                <CardContent className="py-8 px-6 md:py-12 md:px-12 flex flex-col h-full text-center md:text-left">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-8 md:mb-10">
                    <div>
                      ...
                      <p className="text-lg md:text-xl text-muted-foreground font-medium">Com a ImoPonto, poupa:</p>
                      <p className="text-4xl sm:text-5xl md:text-8xl font-black text-accent tracking-tighter my-2 drop-shadow-sm">
                  ...
                  <div className="space-y-4 md:space-y-6 mb-8 md:mb-12">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                      {lifestyleBenefits.filter(b => b.cost <= totalAgencyCost).map((benefit, i) => (
                        <div key={i} className="flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-xl md:rounded-2xl bg-white/50 border border-accent/10 backdrop-blur-sm hover:border-accent/30 transition-colors">
                          ...
                          <span className="text-xs md:text-sm font-bold text-foreground leading-tight text-left">{benefit.label}</span>
                        </div>
                      ))}
