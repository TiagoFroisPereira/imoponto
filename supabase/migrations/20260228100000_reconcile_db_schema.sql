...
-- This migration brings the local migration files in sync with the current Supabase Database state.
-- It adds missing columns, functions, and triggers that exist in the DB but were not present in the local migration history.
...
-- (Definitions extracted from current database state)
...
AS $$
BEGIN
  -- Only fire if status changed to 'confirmed'
...
BEGIN
  PERFORM
    extensions.net_http_post(
      url := 'https://wihgrfiuosvpjinkyluu.supabase.co/functions/v1/email-worker',
